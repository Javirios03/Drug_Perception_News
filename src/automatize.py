...
yesterday = (today - datetime.timedelta(days=2)).strftime("%d/%m/%y")
...
for entry in entries:
    ...
    if "title" in entry and yesterday in entry["title"]:
        url = entry.get("url")
        break
...
if not url:
    ...
    raise ValueError(f"No URL found for the date {yesterday}.")

# Store the URL in a text file
with open("video_urls.txt", "a") as file:
    file.write(f"{yesterday} - {url}\n")
